...
if __name__ == "__main__":
    ...
    x1 = np.linspace(-1, 1, 31) # map 31 points linearly from -1 to 1 in x1
    y1 = np.linspace(-1, 1, 31) # map 31 points linearly from -1 to 1 in y1
    z1 = np.linspace(-1, 1, 31) # map 31 points linearly from -1 to 1 in z1
    
    t1 = np.linspace(0, 1, 11)  # and the time interval is [0, 1]
    
    b0 = np.array([-1] * 900)   # lower boundary
    b1 = np.array([1] * 900)    # upper boundary
    ...
    xx = np.random.randint(31, size=N_train) / 15 - 1 # interval of -1 to 1
    yy = np.random.randint(31, size=N_train) / 15 - 1 # interval of -1 to 1
    zz = np.random.randint(31, size=N_train) / 15 - 1 # interval of -1 to 1
