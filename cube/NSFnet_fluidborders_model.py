# -*- coding: utf-8 -*-
"""
Created on Sun Jan  9 16:55:19 2022
...
class VPNSFnet:
    ...
    def __init__(self, x0, y0, z0, t0, u0, v0, w0, \
                 xb, yb, zb, tb, ub, vb, wb, \
                     xbe, ybe, zbe, tbe, ube, vbe, wbe, \
                         x, y, z, t, layers, ExistModel=0, nnDir=''):
        X0temp = np.concatenate([x0, y0, z0, t0], 1)
        Xbtemp = np.concatenate([xb, yb, zb, tb], 1)
        Xbetemp = np.concatenate([xbe, ybe, zbe, tbe], 1)
        ...
        self.xbe = Xbetemp[:, 0:1]
        self.ybe = Xbetemp[:, 1:2]
        self.zbe = Xbetemp[:, 2:3]
        self.tbe = Xbetemp[:, 3:4]
        ...
        self.xb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.xbe.shape[1]])
        self.yb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.ybe.shape[1]])
        self.zb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.zbe.shape[1]])
        self.tb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.tbe.shape[1]])
        self.ub_empty_tf = tf.placeholder(tf.float32, shape=[None, self.ube.shape[1]])
        self.vb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.vbe.shape[1]])
        self.wb_empty_tf = tf.placeholder(tf.float32, shape=[None, self.wbe.shape[1]])
        ...
        # pred empty boundary cond
        self.u_empty_pred, self.v_empty_pred, self.w_empty_pred, self.p_empty_pred = \
            self.net_NS(self.xb_empty_tf, self.yb_empty_tf, self.zb_empty_tf, self.tb_empty_tf)
        ...
        # empty boundary cond loss
        self.lbe = tf.reduce_mean(tf.square(self.ub_empty_tf - self.u_empty_pred)) + \
            tf.reduce_mean(tf.square(self.vb_empty_tf - self.v_empty_pred)) + \
                tf.reduce_mean(tf.square(self.wb_empty_tf - self.w_empty_pred))
    ...
    def load_NN(self, fileDir, layers):
        ...
        with open(fileDir, 'rb') as f:
            load_weights, load_biases = pickle.load(f)

            # Stored model must has the same # of layers
            assert num_layers == (len(load_weights)+1)

            for num in range(0, num_layers - 1):
                W = tf.Variable(load_weights[num])
                b = tf.Variable(load_biases[num])
                weights.append(W)
                biases.append(b)
    ...
    def net_f_NS(self, x, y, z, t):
        ...
        # f_w = w_t + (u * w_x + v * w_y + w * w_z) + p_z - 1/Re * (w_xx + w_yy + w_zz) #- 10 # gravidade
        
        # no diff
        f_u = u_t + (u * u_x + v * u_y + w * u_z) + p_x
        f_v = v_t + (u * v_x + v * v_y + w * v_z) + p_y
        f_w = w_t + (u * w_x + v * w_y + w * w_z) + p_z
    ...
    def Adam_train(self, nIter=5000, learning_rate=1e-3):
        ...
        tf_dict = {self.x_ini_tf: self.x0, self.y_ini_tf: self.y0, self.z_ini_tf: self.z0, self.t_ini_tf: self.t0,
                   ...
                   self.x_boundary_tf: self.xb, self.y_boundary_tf: self.yb, self.z_boundary_tf: self.zb, self.t_boundary_tf: self.tb, 
                   self.u_boundary_tf: self.ub, self.v_boundary_tf: self.vb, self.w_boundary_tf: self.wb, 
                   self.xb_empty_tf: self.xbe, self.yb_empty_tf: self.ybe, self.zb_empty_tf: self.zbe, self.tb_empty_tf: self.tbe, 
                   self.ub_empty_tf: self.ube, self.vb_empty_tf: self.vbe, self.wb_empty_tf: self.wbe,
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                self.lemptyIterations.append(self.sess.run(self.lbe, tf_dict))
    ...
    def plotLoss(self):
        ...
        ax.set_title('Loss')
        plt.savefig('../figures/cube_01/loss.png', dpi=300, format='png')
        plt.show()
        
        plt.plot(loss_x, self.leIterations)
        plt.title('Le loss')
        plt.savefig('../figures/cube_01/le_loss.png', dpi=300, format='png')
        plt.show()
        
        
        plt.plot(loss_x, self.liIterations, '--g', label = 'li')
        plt.plot(loss_x, self.lbIterations, '-b', label = 'lb')
        plt.plot(loss_x, self.lemptyIterations, ':r', label = 'lbe')
        plt.title('Li, Lb, and Lbe losses')
        plt.legend()
        plt.savefig('../figures/cube_01/li_lb_losses.png', dpi=300, format='png')
        plt.show()
    ...
    def getloss(self):
        ...
        loss = self.sess.run(self.lossIterations, tf_dict)
        loss_lb = self.sess.run(self.lbIterations, tf_dict)
        loss_lbe = self.sess.run(self.lemptyIterations, tf_dict)
        loss_li = self.sess.run(self.liIterations, tf_dict)
        loss_le = self.sess.run(self.leIterations, tf_dict)
        
        # self.plotLoss()
        # self.sess.run(self.plotLoss(), tf_dict)

        return loss, loss_lb, loss_lbe, loss_li, loss_le